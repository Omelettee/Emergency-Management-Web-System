...
session_start();
$_SESSION['username'] = 'cityofatlanta'; // TO_DO: delete this line
...
if (isset($_POST['submit'])) {
    ...
    $query = "SELECT Resource.resourceID AS ID, Resource.name AS Name, Resource.status, User.username, User.name AS Owner, ".
    ...
    if (!empty($incident) && !empty($location)) {
        if (!is_numeric($location) || $location <= 0) {
            print '<p class="error">Error: Provide positive number </p>';
            exit();
        }
        ...
        $query = "SELECT Resource.resourceID AS ID, Resource.name AS Name, Resource.status, User.username, User.username, User.name AS Owner, ".
            "(CONCAT('$', Resource.costAmount, '/', Resource.costUnit_R)) AS Cost, ".
            "Resource.status AS Status, Resource.availableDate AS NextAvailable, ".
                    ...
                    <th class='resultheader'> Action</th>";
                            ...
                            while ($row = mysqli_fetch_array($result)){
                                ...
                                window.open("repair.php?ID=' . $row['ID'] . '","Ratting","width=550,height=170,left=150,top=200,toolbar=0,status=0,");></td>';
                                            ...
                                            // resource owned, available and has no scheduled repair: show deploy and repair button
